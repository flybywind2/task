...
- **키보드 단축키**: Ctrl+Enter로 빠른 저장, ESC로 취소

### 🎨 **시각화 및 뷰**
- **칸반보드**: 드래그 앤 드롭으로 상태 변경 가능한 직관적인 보드
  - **미니 카드 모드**: 간소화된 카드 표시로 더 많은 할일을 한눈에 확인
  - **레이블 그룹핑**: 레이블별로 할일을 그룹화하여 표시
- **리스트뷰**: 테이블 형태의 상세한 할일 관리
  - **인라인 편집**: 제목 더블클릭으로 빠른 편집
  - **드롭다운 수정**: 상태, 카테고리, 우선순위를 클릭하여 즉시 변경
...
### 💻 **데스크톱 애플리케이션**
- **EXE 실행파일**: Windows용 독립 실행 가능한 애플리케이션
- **시스템 트레이**: 백그라운드 실행 및 시스템 트레이 아이콘
- **Windows 알림**: 서버 시작/종료 시 네이티브 알림
- **로그 파일**: 디버깅을 위한 상세한 로그 기록
- **자동 브라우저 실행**: 서버 시작 시 자동으로 브라우저 열기
...
- `PUT /api/tasks/{task_id}/category` - 카테고리 변경
- `PUT /api/tasks/{task_id}/priority` - 우선순위 변경
